...
// 5. Actually size the pair elements, insert gutters and attach event listeners.
    var Split = function (ids, options) {
        ...
        var parentFlexDirection = "";
        if(global.getComputedStyle){
            //优先使用W3C规范
            parentFlexDirection = global.getComputedStyle(parent).flexDirection;
        }else{
            //针对IE9以下兼容
            parentFlexDirection = parent.style.flexDirection;
        }
        //---------xg-----------
        var hideShows = document.createElement("button");
        hideShows.className="buttons icon-angle-left";
        var myHideShow = parent.children[1];
        // myHideShow.appendChild(hideShows);
        document.getElementById("colRight").appendChild(hideShows);
        ...
        //----------xg-----------
        var sizess = options.sizess;
        var buttons = document.querySelector('.buttons');
        var hideShow = options.hideShow !== false ? show():hide();
        //-----------------------
        ...
        var gutter = options.gutter || (function (i, gutterDirection) {
            ...
            gut.className = "col-gutter gutter-" + gutterDirection;
            return gut
        });
        ...
        function setElementSize (el, size, gutSize) {
            ...
            //--------xg------------
            if(el == widthThree && size < 0.3){
                buttons.className = "buttons icon-angle-right";
            }
            if(el == widthThree && size > 0.3){
                buttons.className = "buttons icon-angle-left";
            }
            $(".buttons.icon-angle-left")
            //----------------------
            // eslint-disable-next-line no-param-reassign
            Object.keys(style).forEach(function (prop) { return (el.style[prop] = style[prop]); });

            // $(".icon-angle-left-auto")[0].style.display="none";
        }
        ...
        var widthThree = parent.childNodes[1];
        function stopDragging () {
            var self = this;
            var a = elements[self.a].element;
            var b = elements[self.b].element;
            var videoWebControl=window.parent.document.videoWebControl;
            var vParWidth=$("#videoWindow").width();
            var vParHeight=$("#videoWindow").height();
            if(videoWebControl){
                videoWebControl.JS_Resize(vParWidth, vParHeight);
            }
        ...
        if(buttons.addEventListener){
            buttons.addEventListener("click",buttonsOperate,false);
        }else{
            buttons.attachEvent("onclick",buttonsOperate);
        }
        function buttonsOperate(){
            var videoWebControl=window.parent.document.videoWebControl;
            var pWidth=$("#colLeft").width();
            var vWidth=$("#videoWindow").width();
            var vHeight=$("#videoWindow").height();
            var gWidth=5;//自定义，控件层与背景层width不一致
            if(buttons.className=="buttons icon-angle-left"){
                setSizes(sizess);
                buttons.className="buttons icon-angle-right";
                if(minSize[1] > 0 || minSize > 0){
                    parent.querySelector('.col-gutter').style.display = "none";
                    vWidth=vWidth+pWidth+gWidth;
                    colWidth=pWidth;
                }
            }else{
                setSizes(sizes);
                buttons.className="buttons icon-angle-left";
                parent.querySelector('.col-gutter').style.display = "block";
                colWidth=$("#colLeft").width();
                vWidth=vWidth-colWidth-gWidth;
            }
            // fixedTable();
            if(videoWebControl){
                videoWebControl.JS_Resize(vWidth, vHeight);
            }
        }
        //----------------------
